...
namespace WA\Config\E2E {
    ...
    // solved by using composer in dev environement
    ...
    class E2E_EnsureAdminConfigPanelCest
    {
        ...
        // public function beforeAllTests(AcceptanceTester $I): void
        {
            ...
            && strlen($eConfigOpts[$key])) {
                $testUsersList = wp_kses_post($eConfigOpts[$key]);
            }
        ...
        public function testingWAConfig_paramsSaveFromAdmin(AcceptanceTester $I, Example $example): void
        {
            ...
            $footerTemplateId = '.wa_e_config_opts_wa_footer_template_fr_FR';
            $customFrontHeadId = '.wa-suggest-list-api-frontheads';
            ...
            // Call to undefined method AcceptanceTester::executeJS => with WebBrowser or selenium only ?
            // $valMethode1 = $I->executeJS("return $('$footerTemplateId').val()");
            // $valMethode2 = $I->executeJS("return jQuery('$footerTemplateId').val()");
            // $this->debug("Grab from JS ok in PhpBrowser ?", $valMethode1, $valMethode2);
            // $valMethode3 = $I->grabAttributeFrom($footerTemplateId, 'innerHTML');
            // $valMethode3 = $I->grabAttributeFrom($footerTemplateId, 'value');
            // $valMethode3 = $I->grabAttributeFrom($footerTemplateId, 'textContent');
            // $valMethode3 = $I->grabTextFrom($footerTemplateId); // OK, BUT TEXT MODE only...
            // $this->debug("Grab from PhpBrowser ?", $valMethode3);

            // $nodes = $I->match($footerTemplateId); // Protected, for helpers only, not for test files...
            // $I->assert($nodes->count() === 1, "$footerTemplateId should be found and unique");
            // $this->debug("Node match : ", $nodes->first());
            // // $I->makeElementScreenshot($footerTemplateId, 'debugInProgress_footerTemplateId'); // For WebDriver only...

            // $srcContent = $I->getModule('PhpBrowser')->_getResponseContent(); // Only work in helpers
            // $this->debug("Custom extract : ", $srcContent);

            // OK, WORKING, BUT HTLM Syntax was in cause. Juste need 
            // to encode textarea content with htmlspecialchars() to make it
            // work ok with regular 
            // $srcContent = $I->grabHtmlFrom($footerTemplateId); // OK, work with PhpBrowser
            // $this->debug("Custom HTML extract : ", $srcContent);

            $I->seeElement($footerTemplateId);

            // https://stackoverflow.com/questions/46135303/undefined-property-domelementwholetext
            // TODO : Textarea need some normalisation to work with regular value check ? 
            //        or new method for this job not yet discovered ?
            $initialFTemplate = $I->grabValueFrom($footerTemplateId);
            // $initialFTemplate = $I->grabHtmlFrom($footerTemplateId);
            // grabValueFrom Just need encode textarea content with htmlspecialchars()
            // WRONG HTML => will give wrong localized error, wrong html format give wrong errors...
            $I->comment("Grab '$initialFTemplate' from '$footerTemplateId'");
            $I->fillField($footerTemplateId, ''); // Clean to avoid clash with initial FCredit
            // $I->fillContentWithHtml($footerTemplateId, ''); // Clean to avoid clash with initial FCredit

            $initialCustomFrontHead = $I->grabValueFrom($customFrontHeadId);
            $I->comment("Grab '$initialCustomFrontHead' from '$customFrontHeadId'");
            $I->fillField($customFrontHeadId, ''); // Clean to avoid clash with initial FCredit
            ...
            $this->rollback[] = function () use (
                $I,
                $initialFCredit,
                $footerCreditId, $footerTemplateId, $customFrontHeadId,
                $initialEnableFooter, $initialFTemplate, $initialCustomFrontHead,
                $footerEnableId
            ) {
                ...
                $I->fillField($customFrontHeadId, $initialCustomFrontHead);
                $I->fillField($footerTemplateId, $initialFTemplate);
                // $I->fillContentWithHtml($footerTemplateId, $initialFTemplate);
                $I->click('#submit');
            ...
            $I->amOnPage('/');
            $I->makeHtmlSnapshot('params-save-from-admin-result-page-' . microtime(true));
        ...
        public function testingWAConfig_emailTrackingForComments(AcceptanceTester $I, Example $example): void
        {
            ...
            wp_cache_delete("alloptions", "options"); // avoid wrong value due to cached stuff
            $E2ETestsOptions = get_option($eConfigE2ETestsOptsKey, []);
        ...
        protected function authenticateUser($I, $idx = 0) {
            ...
            $I->makeHtmlSnapshot('sendAjaxPostRequest_wa-e2e-test-action_authenticate-user-' . microtime(true));
            ...
            // if ($I) {
            //     $I->amOnPage('/wp-admin');
            //     $I->makeHtmlSnapshot('wp-admin_wa-e2e-test-action_after-authenticate-user-' . microtime(true));
        ...
        protected function logoutUser($I, $userLogin) {
            ...
            $I->makeHtmlSnapshot('htmlResult_before_logout-user-' . microtime(true));
            // $I->haveHttpHeader("Content-Type", "application/x-www-form-urlencoded");
            $postData = [
                'action' => 'wa-e2e-test-action',
                'wa-access-hash' => $this->accessHash,
            ...
            //       $I->makeHtmlSnapshot('htmlResult_before_logout-user-' . microtime(true)); (Or not, sound like
            //       snapshoot of last success result... 
            //       check wp-content/debug.log instead... => wp_die with page not found error...)
            $I->makeHtmlSnapshot('sendAjaxPostRequest_wa-e2e-test-action_logout-user-' . microtime(true));
        }
        ...
        protected function panelsProvider() // alternatively, if you want the function to be public, be sure to prefix it with `_`
        {
            return [
                [
                    ...
                    'title' => __('Documentation', 'monwoo-web-agency-config'/** 📜*/),
                    'url' => "/wp-admin/admin.php?page=wa-e-config-doc-page"
                ], [
                    'title' => __('Revue qualité', 'monwoo-web-agency-config'/** 📜*/),
